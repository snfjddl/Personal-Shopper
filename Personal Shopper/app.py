...
app = Flask(__name__)
app.config['UPLOAD_DIR'] = r'C:\Users\tjrdl\Downloads\final_web\static\upload_files'
...
def mains():
    return flask.render_template('mains.html')
...
def make_prediction():
    if request.method == 'POST':
        ...
        f.save(path)
        category, color = class_model.predict_img(path)
        # detect_img(path)
        style = style_model.style_return(path)
        print(path)
        print(category)
        print(color)
        print(style)

        paths = "/static/upload_files/" + fname
        return render_template('predict_result.html', img_path=paths, category=category, color=color, style=style)
...
if __name__ == '__main__':
    # 라벨링
    # labels = json.load(open(r'C:\Users\tjrdl\Downloads\Flask_ML&DL\labels.txt'))
    # label_dict = {}
    # filenames = list(labels)
    #
    # for label_type in ['material', 'category', 'color']:
    #     label_dict[label_type] = {}
    #     for idx, label in enumerate(set([labels[f][label_type] for f in labels])):
    #         label_dict[label_type][idx] = label
    #         label_dict[label_type][label] = idx
    #
    # # 모델 로드
    # # ml/class_model.py 선 실행 후 생성
    # label_type = 'color'
    # load_model_color = generate_model(label_type)
    # load_model_color.load_state_dict(torch.load(r'C:\Users\tjrdl\Downloads\Flask_ML&DL\model\model_color.pth', map_location=torch.device('cpu')))
    # load_model_color.eval()

    # Flask 서비스 스타트
    app.run(host='192.168.47.1', port=8000, debug=False)
